...
namespace Core_Bootcamp.Controllers
{
    public class EmployeeTestController : Controller
    {
        ...
        {
            var httpClient = new HttpClient();
            var responseMessage = await httpClient.GetAsync("https://localhost:44348/api/Default/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> EditEmployee(Class1 p)
        {
            ...
            var responseMessage = await httpClient.PutAsync("https://localhost:44348/api/Default", content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var httpClient = new HttpClient();
            var responseMessage = await httpClient.DeleteAsync("https://localhost:44348/api/Default/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
